...
exports.create = (req, res, next) => {
    ...
    const formTest_ = JSON.parse(req.body.formTest);
    if(req.files) {
        formTest_.imageUrls = req.files.map(file => `${req.protocol}://${req.get('host')}/${file.path}`);
    }

    const formTest = new FormTest({...formTest_});
